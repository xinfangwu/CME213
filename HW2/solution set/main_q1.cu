...
void initialize_array(vec &input_array, size_t arr_size)
{
  ...
  {
    elem_type entry = static_cast<elem_type>(rand()) /
                          (static_cast<elem_type>(RAND_MAX / 2.0)) -
                      1.0;
    input_array.push_back(entry);
  }
}
...
protected:
  ...
  // You can make the graph for Q4,Q5 more easily by saving this array as a
...
TEST_F(RecurrenceTestFixture, InitializeArrayTest_2)
{
  initialize_array(init_arr, MAX_ARR_SIZE);
  check_initialization(init_arr, MAX_ARR_SIZE);
}
...
TEST_F(RecurrenceTestFixture, RecurrenceKernelTest_3)
{
  ...
  // Testing accuracy of Recurrence Kernel (Note 4x4 might not cover array size)
  size_t num_iter = 2;
...
// No code changes necessary
TEST_F(RecurrenceTestFixture, RecurrenceItersTest_5)
{
  /*
   * ––––––––––-----------------------------
   * Question 1.5: vary number of iterations
   * ––––––––––-----------------------------
   */
  // init array
  initialize();
