...
public:
  ...
  hyperparameters(int input_size, int hidden_size, int num_classes, 
                  nn_real reg, nn_real learning_rate, int N, int num_epochs, 
                  int batch_size, int debug)
    : input_size(input_size), hidden_size(hidden_size), 
      num_classes(num_classes), learning_rate(learning_rate), N(N), 
      num_epochs(num_epochs), batch_size(batch_size), debug(debug) {
    assert(input_size > 0);
 ...
 */
struct GPUGrads {
  std::vector<DeviceMatrix> dW; // The gradients of the weights for each layer
  std::vector<DeviceMatrix> db; // The gradients of the biases for each layer
  std::shared_ptr<DeviceAllocator> gpu_mem_pool; // GPU memory for gradients
  size_t total_elements;        /* Number of elements allocated for storing the 
                                   GPU gradients. */
