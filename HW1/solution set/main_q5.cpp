...
std::vector<T> daxpy(T a, const std::vector<T> &x, const std::vector<T> &y) {
  ...
  std::transform(x.begin(), x.end(), y.begin(), result.begin(),
                 [&a](T x_i, T y_i) { return a * x_i + y_i; });
  return result;
}
...
bool all_students_passed(const std::vector<Student> &students,
  ...
  return std::all_of(students.begin(), students.end(),
                     [&pass_threshold](const Student &s) {
                       double pct = (HOMEWORK_WEIGHT * s.homework) +
                                    (MIDTERM_WEIGHT * s.midterm) +
...
void sparse_matrix_sort(std::list<SparseMatrixCoordinate<T>> &list) {
  // METHOD 1: Call std::list::sort() with custom predicate.
  list.sort([](const SparseMatrixCoordinate<T> &a,
               const SparseMatrixCoordinate<T> &b) {
    return (a.row < b.row) || (a.row == b.row && a.col < b.col);
  });

  // METHOD 2: Implement < operator in the SparseMatrixCoordinate struct.
  // This removes the need for a custom predicate.
  // list.sort();
}
