...
GPUGrads::GPUGrads(const NeuralNetwork &nn)
{
  ...
  gpu_mem_pool =
      std::make_shared<DeviceAllocator>(total_size);
  ...
  for (int i = 0; i < nn.num_layers; i++)
  {
    assert(offset < total_size); // Check if we are within bounds
      ...
      num_procs(num_procs)
{
  ...
  for (int i = 0; i < nn.num_layers; i++)
  {
    ...
    cache.z[i] = DeviceMatrix(nn.W[i].n_rows, batch_size);
    cache.a[i] = DeviceMatrix(nn.W[i].n_rows, batch_size);
    // cache.z[i] = DeviceMatrix(nn.W[i].n_rows, 1);
    // cache.a[i] = DeviceMatrix(nn.W[i].n_rows, 1);
...
nn_real DataParallelNeuralNetwork::loss(const DeviceMatrix &y, nn_real weight)
{
  ...
  DCELoss(cache.a[1], y, ce_sum_mat_gpu);
...
void DataParallelNeuralNetwork::backward(const DeviceMatrix &y,
                                         nn_real grad_weight)
{
  DeviceMatrix diff = cache.yc; // shallow copy
